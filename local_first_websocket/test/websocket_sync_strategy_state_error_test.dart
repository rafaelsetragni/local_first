...
void main() {
  ...
  group('WebSocketSyncStrategy - StateError Coverage', () {
    ...
    test('should handle StateError when sending pong response (lines 291, 293)',
        () async {
    ...
    test('should retry authentication after successful credential refresh (lines 402, 408, 414, 416)',
        () async {
      final messageController = StreamController<dynamic>.broadcast();
      final mockChannel = MockWebSocketChannel();
      final mockSink = MockWebSocketSink();
    ...
    test('should handle StateError during heartbeat ping (lines 532, 539)',
        () async {
      final messageController = StreamController<dynamic>.broadcast();
      final mockChannel = MockWebSocketChannel();
      final mockSink = MockWebSocketSink();
    ...
    test('should handle DateTime object in timestamp extraction (line 594)',
        () async {
      final messageController = StreamController<dynamic>.broadcast();
      final mockChannel = MockWebSocketChannel();
      final mockSink = MockWebSocketSink();
      ...
      final eventsMessage = jsonEncode({
        ...
        'events': [
          ...
          {
            'eventId': 'event-2',
            LocalFirstEvent.kSyncCreatedAt: now.add(Duration(hours: 1)).toIso8601String(),
            'data': {'id': '2'}
          }
        ],
      });
    ...
    test('should throw StateError when calling _sendMessage without connection (line 431)',
        () async {
      final strategy = WebSocketSyncStrategy(
        websocketUrl: 'ws://localhost:8080/test',
        onBuildSyncFilter: (_) async => null,
