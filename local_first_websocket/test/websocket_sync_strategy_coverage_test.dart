...
void main() {
  ...
  group('WebSocketSyncStrategy - Coverage Tests', () {
    ...
    test('should send event immediately when connected (cover lines 358, 364)',
        () async {
    ...
    test('should flush pending queue with batch messages (cover line 441)',
        () async {
    ...
    test('should handle DateTime object in timestamp extraction (cover line 496)',
        () async {
      final messageController = StreamController<dynamic>.broadcast();
      final mockChannel = MockWebSocketChannel();
      final mockSink = MockWebSocketSink();
      ...
      final jsonString = jsonEncode(eventsData, toEncodable: (obj) {
        if (obj is DateTime) {
    ...
    test(
      ...
      final eventsMessage = jsonEncode({
        ...
        'events': [
          {
            'eventId': 'event-1',
            LocalFirstEvent.kSyncCreatedAt: now.subtract(Duration(hours: 3)).toIso8601String(),
            'data': {'id': '1'}
          },
          {
            'eventId': 'event-2',
            LocalFirstEvent.kSyncCreatedAt: now.subtract(Duration(hours: 2)).toIso8601String(),
            'data': {'id': '2'}
          },
          {
            'eventId': 'event-3',
            LocalFirstEvent.kSyncCreatedAt: now.subtract(Duration(hours: 1)).toIso8601String(),
            'data': {'id': '3'}
          },
        ],
      });
    ...
    test('should call onSyncCompleted callback after receiving events',
        () async {
      final List<StreamController<dynamic>> controllers = [];
