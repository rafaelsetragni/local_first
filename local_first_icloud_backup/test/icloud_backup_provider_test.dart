...
class MockICloudStorageDelegate extends Mock
    implements ICloudStorageDelegate {}
...
void main() {
  ...
  group('ICloudBackupProvider', () {
    ...
    group('upload', () {
      test('writes temp file, uploads, cleans up, and returns metadata',
          () async {
        when(() => mockDelegate.upload(
              containerId: any(named: 'containerId'),
              filePath: any(named: 'filePath'),
              destinationRelativePath:
                  any(named: 'destinationRelativePath'),
            )).thenAnswer((_) async {});
        ...
        final captured = verify(() => mockDelegate.upload(
              containerId: captureAny(named: 'containerId'),
              filePath: captureAny(named: 'filePath'),
              destinationRelativePath:
                  captureAny(named: 'destinationRelativePath'),
            )).captured;
    ...
    group('download', () {
      test('downloads to temp file, reads bytes, cleans up', () async {
        when(() => mockDelegate.download(
          ...
          final destPath = invocation.namedArguments[#destinationFilePath]
              as String;
